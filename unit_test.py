...
from layer import FullyConnectedLayer, SoftMaxLayer, Convolution2DLayer, EmbeddingLayer, MaxoutLayer, HighwayNetworkLayer
from utils import get_data
from recurrent import RecurrentNetwork, RNN, LSTM, BiRNN, BiLSTM, FastLSTM

import numpy as np
import pickle, gzip

def unit_test_mlp(dropout_test=False, maxout_test=False, optimizer='sgd'):
	...
	if dropout_test:
		network.add(FullyConnectedLayer(train_x.shape[1], 500, activation='tanh', dropout_rate=0.9))
		network.add(FullyConnectedLayer(500, 10, activation='tanh', dropout_rate=0.9))
	elif maxout_test:
		network.add(MaxoutLayer(train_x.shape[1], 26, dropout_rate=0.9))
		network.add(MaxoutLayer(30, 3, dropout_rate=0.9))
	else:
		network.add(FullyConnectedLayer(train_x.shape[1], 500, activation='tanh'))
		network.add(FullyConnectedLayer(500, 500, activation='tanh'))
...
def unit_test_highway_network():

	train_x, train_y, dev_x, dev_y, test_x, test_y = get_data(dataset='mnist')
	network = SequentialNetwork(input_type='2d', output_type='multiple_class')

	network.add(HighwayNetworkLayer(train_x.shape[1], 50, activation='relu'))
	network.add(HighwayNetworkLayer(50, 50, activation='relu'))
	network.add(HighwayNetworkLayer(50, 50, activation='relu'))
	network.add(HighwayNetworkLayer(50, 50, activation='relu'))
	network.add(HighwayNetworkLayer(50, 50, activation='relu'))
	network.add(HighwayNetworkLayer(50, 50, activation='relu'))
	network.add(HighwayNetworkLayer(50, 10, activation='relu'))
	network.add(SoftMaxLayer(hierarchical=False))
	...
	for i in range(100):
		...
		print 'Epoch : %d ' %(i * 2)
		network.train(train_x, train_y, nb_epochs=2)
		print 'Accuracy on dev : %f ' %((np.argmax(network.predict(dev_x), axis=1) == dev_y).mean())
		print 'Accuracy on test : %f ' %((np.argmax(network.predict(test_x), axis=1) == test_y).mean())
